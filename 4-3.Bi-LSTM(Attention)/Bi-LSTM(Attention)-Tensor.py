...
n_step = 3  # all sentence is consist of 3 words (word embedding not char)
n_class = 2  # 0 or 1
...
# Attention  # output : [batch_size, len_seq, n_hidden]
output = tf.concat([output[0], output[1]], 2)  # output[0] : lstm_fw, output[1] : lstm_bw
final_hidden_state = tf.concat([final_state[1][0], final_state[1][1]],
                               1)  # final_hidden_state : [batch_size, n_hidden * num_directions(=2)]
final_hidden_state = tf.expand_dims(final_hidden_state,
                                    2)  # final_hidden_state : [batch_size, n_hidden * num_directions(=2), 1]
...
context = tf.matmul(tf.transpose(output, [0, 2, 1]),
                    tf.expand_dims(soft_attn_weights, 2))  # context : [batch_size, n_hidden * num_directions(=2), 1]
...
logs_path = '/tmp/tensorflow_logs/bi-lstm_attention'
# Training
with tf.Session() as sess:
    ...
    for epoch in range(5000):
        ...
        _, loss, attention, summary = sess.run([optimizer, cost, soft_attn_weights, merged_summary_op], feed_dict={X: input_batch, Y: target_batch})

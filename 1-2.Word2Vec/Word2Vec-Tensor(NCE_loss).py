...
voc_size = len(word_list)  # voc_size 为词汇表, the set of unique words


def random_batch(data, size):
    """
    @author: yinshuai  training batch generate
    :param data:
    :param size:
    :return: random_inputs: 从数据集中随机选择出来的输入数据
             random_labels: 对应输入数据的,从数据集中随机选择出来的标签
    """
    random_inputs = []
    random_labels = []
    random_index = np.random.choice(range(len(data)), size, replace=False)   # 从所有的数据行中,随机选择出size个索引
...
skip_grams = [] # 存储所有滑动窗口出来的训练数据(N-Gram)
for i in range(1, len(word_sequence) - 1):
    target = word_dict[word_sequence[i]]
    context = [word_dict[word_sequence[i - 1]],
               word_dict[word_sequence[i + 1]]]  # 滑动窗口的大小为3 [word_sequence[i-1], word_sequence[i], word_squence[i+1]]

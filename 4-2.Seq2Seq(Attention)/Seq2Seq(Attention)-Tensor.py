# code by yinshuai
# seq to seq translate model using attention
import matplotlib.pyplot as plt
import numpy as np
import tensorflow as tf

tf.reset_default_graph()
# S(<start>): Symbol that shows starting of decoding input
# E(<end>): Symbol that shows starting of decoding output
# P(<padding>): Symbol that will fill in blank sequence if current batch data size is short than time steps
...
def make_batch(sentences):
    """
    使用将输入,输出,目标数据进行one-hot化
    :param sentences: 输入句子
    :return: tuple: Type-> Tuple
                    input_batch: 输入批数据,word is replaced by corresponding id
    """
...
def get_att_score(dec_output, enc_output):  # enc_output [n_step, n_hidden]
    """
    @author: yinshuai 计算输出结果 the shape is [n_step, n_hidden]? not sure
    :param dec_output:
    :param enc_output:
    :return:
    """
...
with tf.variable_scope('encode'):
    enc_cell = tf.nn.rnn_cell.BasicRNNCell(n_hidden)  # n_hidden
...
with tf.variable_scope('decode'):
    ...
    for i in range(n_step):
        ...
        # in attention mode, the contexts contexts context is calculate by the attn_weight * enc_outputs
...
with tf.Session() as sess:
    ...
    for epoch in range(2000):
        input_batch, output_batch, target_batch = make_batch(sentences)  # get one-hot training data
    ...
    predict_batch = [np.eye(n_class)[[word_dict[n] for n in 'P P P P P'.split()]]]  # P here is padding
